...
- [x] **T5.5 Ephemeral runners – ARC + OIDC→AWS**
  - [x] Commit `infra/arc/` manifests for **Actions Runner Controller** runner scale sets; default to ephemeral pods.
  - [x] Configure **OIDC** → AWS roles for temporary credentials (no long-lived secrets).
  - **Acceptance:** workflows execute on ephemeral runners; AWS access uses OIDC with rehearsal evidence archived in `dist/staging/backfill/20251101T171853Z/` and `dist/staging/marquez/20251101T171901Z/cli.log`, referenced in the 2025-11-02 changelog.【F:docs/CHANGELOG.md†L1-L23】
