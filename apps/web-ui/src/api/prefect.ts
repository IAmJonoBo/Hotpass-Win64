...
const prefectLimiter = createRateLimiter(30, 60_000)

const fetchWithLimiter = (url: string, init: RequestInit = {}) =>
  prefectLimiter(() => fetch(url, { credentials: 'include', ...init }))
...
export const prefectApi = {
  ...
  async checkHealth(): Promise<boolean> {
    ...
    try {
      const response = await fetchWithLimiter(url, { headers: { Accept: 'application/json' } })
      if (!response.ok) {
        return false
      }
