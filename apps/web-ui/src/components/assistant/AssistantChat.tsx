...
export function AssistantChat({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I can help with Hotpass operations. Ask me to run refine, trigger QA, plan research, open tunnels, generate env files, or list flows.",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    const allowNetwork =
      lowerText.includes("allow network") || lowerText.includes("with network");
    const disableNetwork =
      lowerText.includes("no network") || lowerText.includes("offline");
    ...
    if (lowerText.includes("run refine")) {
      ...
      lowerText.includes("run enrich") ||
      lowerText.includes("run enrichment")
    ) {
      toolName = "runEnrich";
      if (profile) toolArgs.profile = profile;
      ...
      lowerText.includes("run contract") ||
      lowerText.includes("run contracts") ||
      lowerText.includes("emit contract")
    ) {
      ...
      lowerText.includes("list flows") ||
      lowerText.includes("show flows")
    ) {
      toolName = "listFlows";
    } else if (
      lowerText.includes("list lineage") ||
      lowerText.includes("show lineage")
    ) {
      ...
      toolName = "runOperatorWizard";
      const hostMatch = text.match(/host[:=\s]+([\w.-]+)/i);
      if (hostMatch?.[1]) {
        toolArgs.host = hostMatch[1];
      }
      const viaMatch = text.match(/via[:=\s]+([\w-]+)/i);
      if (viaMatch?.[1]) {
        toolArgs.via = viaMatch[1];
      }
      if (lowerText.includes("skip credentials")) {
        toolArgs.skipCredentials = true;
      }
      if (lowerText.includes("skip setup")) {
        toolArgs.skipSetup = true;
      }
      if (lowerText.includes("skip env") || lowerText.includes("no env")) {
        toolArgs.skipEnv = true;
      }
      if (lowerText.includes("no credentials")) {
        toolArgs.includeCredentials = false;
      }
      if (allowNetwork) {
        toolArgs.allowNetwork = true;
      }
    } else if (
      lowerText.includes("open tunnel") ||
      lowerText.includes("connect tunnel") ||
      lowerText.includes("start tunnel") ||
      lowerText.includes("open bastion")
    ) {
      toolName = "runOperatorConnect";
      const hostMatch = text.match(/host[:=\s]+([\w.-]+)/i);
      if (hostMatch?.[1]) {
        toolArgs.host = hostMatch[1];
      }
      ...
      if (lowerText.includes("detach")) {
        toolArgs.detach = true;
      }
      if (
        lowerText.includes("no marquez") ||
        lowerText.includes("skip marquez")
      ) {
        toolArgs.includeMarquez = false;
      }
      const labelMatch = text.match(/label[:=\s]+([\w-]+)/i);
      if (labelMatch?.[1]) {
        toolArgs.label = labelMatch[1];
      }
    } else if (
      lowerText.includes("generate env") ||
      lowerText.includes("write env") ||
      lowerText.includes("env file")
    ) {
      toolName = "generateEnvFile";
      const targetMatch = text.match(/target[:=\s]+([\w.-]+)/i);
      if (targetMatch?.[1]) {
        toolArgs.target = targetMatch[1];
      }
      const prefectUrlMatch = text.match(/prefect[_-]?url[:=\s]+(\S+)/i);
      if (prefectUrlMatch?.[1]) {
        toolArgs.prefectUrl = prefectUrlMatch[1];
      }
      const openlineageUrlMatch = text.match(
        /openlineage[_-]?url[:=\s]+(\S+)/i,
      );
      if (openlineageUrlMatch?.[1]) {
        toolArgs.openlineageUrl = openlineageUrlMatch[1];
      }
      if (lowerText.includes("no credentials")) {
        toolArgs.includeCredentials = false;
      }
      ...
      (lowerText.includes("open run") || lowerText.includes("show run")) &&
      runIdMatch?.[1]
    ) {
      toolName = "openRun";
      toolArgs.runId = runIdMatch[1];
    } else if (
      lowerText.includes("get runs") ||
      lowerText.includes("flow runs")
    ) {
      ...
      if (toolResult) {
        if (toolResult.success) {
          ...
          if (toolName === "listFlows" && toolResult.data) {
            ...
            const runs = toolResult.data as {
              name: string;
              state_name: string;
            }[];
            responseContent += `\n\nRecent runs:\n${runs
              .slice(0, 5)
              .map((r) => `• ${r.name} - ${r.state_name}`)
              .join("\n")}`;
          } else if (
            isCommandTool &&
            toolResult.data &&
            isCommandToolResultData(toolResult.data)
          ) {
    ...
    <Card className={cn("flex flex-col", className)}>
      ...
      <CardFooter className="border-t p-4 flex-col gap-3">
        ...
        <div className="w-full text-xs text-muted-foreground border-t pt-2 flex items-center justify-between">
          <span>
            Telemetry: {lineageTelemetry?.incompleteFacets ?? 0} pending
            backfills · {flowRuns.length} cached Prefect runs · last sync{" "}
            {formatDistanceToNow(lastPollTime, { addSuffix: true })} ·
            source=marquez
          </span>
          <Badge variant="outline" className="text-xs">
            Env: {environment}
