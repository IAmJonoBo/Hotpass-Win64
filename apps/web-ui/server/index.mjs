...
const prefectTarget =
  ...
  process.env.VITE_PREFECT_API_URL ||
  "http://localhost:4200/api";
const marquezTarget =
  process.env.MARQUEZ_API_URL ||
  process.env.VITE_MARQUEZ_API_URL ||
  process.env.OPENLINEAGE_URL ||
  "http://localhost:5000/api/v1";

const prefectLimit = Number.parseInt(
  process.env.PREFECT_RATE_LIMIT ?? "120",
  10,
);
const marquezLimit = Number.parseInt(
  process.env.MARQUEZ_RATE_LIMIT ?? "60",
  10,
);
const IMPORT_ROOT =
  process.env.HOTPASS_IMPORT_ROOT || path.join(process.cwd(), "dist", "import");
const CONTRACTS_ROOT =
  process.env.HOTPASS_CONTRACT_ROOT ||
  path.join(process.cwd(), "dist", "contracts");
const QA_ROOT =
  process.env.HOTPASS_QA_ROOT ||
  path.join(process.cwd(), "dist", "quality-gates");
const MAX_IMPORT_FILE_SIZE = Number.parseInt(
  process.env.HOTPASS_IMPORT_MAX_FILE_SIZE ?? `${1024 * 1024 * 1024}`,
  10,
);
const MAX_IMPORT_FILES = Number.parseInt(
  process.env.HOTPASS_IMPORT_MAX_FILES ?? "10",
  10,
);
const PIPELINE_RUN_LIMIT = Number.parseInt(
  process.env.HOTPASS_PIPELINE_RUN_LIMIT ?? "100",
  10,
);
const DEFAULT_INVENTORY_MANIFEST = path.join(
  process.cwd(),
  "data",
  "inventory",
  "asset-register.yaml",
);
const DEFAULT_INVENTORY_STATUS = path.join(
  process.cwd(),
  "data",
  "inventory",
  "feature-status.yaml",
...
await mkdir(IMPORT_ROOT, { recursive: true }).catch((error) => {
  console.warn("[bootstrap] unable to prepare import root", {
    IMPORT_ROOT,
    error,
  });
});
...
await mkdir(CONTRACTS_ROOT, { recursive: true }).catch((error) => {
  console.warn("[bootstrap] unable to prepare contracts root", {
    CONTRACTS_ROOT,
    error,
  });
});
...
const isImportJob = (job) =>
  Boolean(job?.metadata && job.metadata.type === "import");
...
const ensureBackendRequirement = (requirements, detail) => {
  ...
  for (const requirement of requirements) {
    ...
    if (
      surface === "backend" ||
      ["backend", "backend-service"].includes(requirement.id)
    ) {
...
const loadInventorySnapshot = async () => {
  const { manifestPath, statusPath, ttlMs } = resolveInventorySettings();
  const now = Date.now();
  const manifestStats = await safeStatFile(manifestPath);
  if (!manifestStats) {
    const error = new Error(`Inventory manifest not found at ${manifestPath}`);
    error.code = "ENOENT";
    throw error;
  }

  const statusStats = await safeStatFile(statusPath);

  if (
    inventoryCache.snapshot &&
    inventoryCache.manifestPath === manifestPath &&
    inventoryCache.statusPath === statusPath &&
  ...
  const rawAssets = Array.isArray(manifestPayload.assets)
    ? manifestPayload.assets
    : [];
  const assets = rawAssets.map(normaliseAssetRecord).filter(Boolean);
  ...
  if (!assets.length) {
    ensureBackendRequirement(
      requirements,
      "Inventory manifest contains no assets",
    );
  }
  ...
  inventoryCache.manifestMtimeMs = manifestStats.mtimeMs;
  inventoryCache.statusMtimeMs = statusStats?.mtimeMs ?? 0;
  inventoryCache.ttlMs = ttlMs;
  inventoryCache.expiresAt = now + ttlMs;
...
const enumerateImportArtifacts = async (job) => {
  ...
  const artifacts = [];
  const outputPath =
    typeof job.metadata.outputPath === "string"
      ? job.metadata.outputPath
      : null;
  const archiveDir =
    typeof job.metadata.archiveDir === "string"
      ? job.metadata.archiveDir
      : null;
  ...
  const profilePath =
    profileAttachment &&
    typeof profileAttachment === "object" &&
    typeof profileAttachment.path === "string"
  ...
  const contractPath =
    contractAttachment &&
    typeof contractAttachment === "object" &&
    typeof contractAttachment.path === "string"
...
const derivePipelineAction = ({ name, parameters, tags, metadataType }) => {
  ...
  const loweredTags = Array.isArray(tags)
    ? tags.map((tag) => String(tag).toLowerCase())
    : [];
  const parameterAction =
    typeof parameters?.action === "string"
      ? parameters.action.toLowerCase()
      : null;
...
const mapPrefectRunToPipeline = (run) => {
  ...
  return {
    ...
    profile:
      typeof parameters.profile === "string" ? parameters.profile : undefined,
    runName: run?.name ?? run?.id ?? undefined,
    notes: typeof parameters.notes === "string" ? parameters.notes : undefined,
    dataDocsUrl:
      typeof parameters.data_docs_url === "string"
        ? parameters.data_docs_url
        : undefined,
    metadata: {
...
const mapJobToPipeline = (job) => {
  ...
  return {
    ...
    updatedAt:
      ...
      null,
    profile:
      typeof metadata.profile === "string" ? metadata.profile : undefined,
    runName: job?.label ?? job?.command?.join(" ") ?? undefined,
    notes: typeof metadata.notes === "string" ? metadata.notes : undefined,
    dataDocsUrl:
      typeof metadata.dataDocsUrl === "string"
        ? metadata.dataDocsUrl
        : undefined,
    metadata: {
...
const fetchRecentPrefectRuns = async (limit) => {
  const resolvedLimit =
    Number.isFinite(limit) && limit > 0
      ? Math.min(limit, PIPELINE_RUN_LIMIT)
      : 50;
  const base = prefectTarget.endsWith("/")
    ? prefectTarget.slice(0, -1)
    : prefectTarget;
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        "[pipeline-runs] Prefect returned non-200 response",
        response.status,
      );
      return [];
    }
...
app.post("/api/imports/profile", async (req, res) => {
  ...
  const maxRows =
    req.query.maxRows !== undefined
      ? parseInteger(req.query.maxRows, undefined)
      : undefined;
  ...
  const profileAndRespond = async (workbookPath) => {
    try {
      ...
      const profile = await runImportProfiler(workbookPath, {
        sampleRows,
        maxRows,
      });
      res.json({ profile });
      ...
      if (error.code === "ENOENT") {
        ...
        sendError(res, 500, "Failed to profile workbook", {
  ...
  if (req.is("application/json")) {
    ...
    if (!resolved.startsWith(process.cwd())) {
      return sendError(
        res,
        400,
        "workbookPath must be within the project directory",
      );
    }
  ...
  if (!contentType.includes("multipart/form-data")) {
    return sendError(
      res,
      415,
      "Unsupported content type. Use multipart/form-data or JSON.",
    );
  }
  ...
  try {
    ...
    return sendError(res, 500, "Unable to initialise upload workspace", {
      message: error.message,
    });
  }
  ...
  busboy.on("finish", async () => {
    try {
      if (streamError) {
        ...
        sendError(res, 400, "Failed to receive workbook", {
...
app.get("/api/imports/profiles", async (_req, res) => {
  try {
    ...
    sendError(res, 500, "Unable to load stored profiles", {
      message: error.message,
    });
  }
});
...
app.get("/api/imports/profiles/:id", async (req, res) => {
  ...
  try {
    ...
    sendError(res, 500, "Unable to load stored profile", {
      message: error.message,
    });
  }
});
...
app.post("/api/imports/profiles", csrfProtection, async (req, res) => {
  ...
  const entry = {
    ...
    workbookPath:
      typeof req.body.workbookPath === "string"
        ? req.body.workbookPath
        : undefined,
    description:
  ...
  try {
    ...
    sendError(res, 500, "Unable to persist profile", {
      message: error.message,
    });
  }
});
...
app.get("/api/imports/consolidation/telemetry", async (_req, res) => {
  try {
    ...
    sendError(res, 500, "Unable to load consolidation telemetry", {
      message: error.message,
    });
  }
});
...
app.get("/api/imports/templates/:id/summary", async (req, res) => {
  ...
  try {
    ...
    sendError(res, 500, "Unable to summarise template", {
      message: error.message,
    });
  }
});
...
app.get("/api/contracts/:name/download", async (req, res) => {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      ...
      sendError(res, 500, "Unable to download contract", {
        message: error.message,
      });
    }
  }
  ...
  try {
    ...
    for (const contract of contracts) {
      ...
      try {
        ...
        console.warn("[contracts] failed to inspect contract for rule", {
          ruleId,
          filePath,
          error,
        });
    ...
    sendError(res, 500, "Unable to resolve contract rule reference", {
      message: error.message,
    });
  }
});
...
app.get("/api/qa/latest", async (_req, res) => {
  try {
    ...
    const dataDocsPath =
      typeof summary?.data_docs === "string"
        ? path.relative(process.cwd(), summary.data_docs)
        : null;

    res.json({
      summary,
      dataDocsPath: dataDocsPath
        ? `/${dataDocsPath.split(path.sep).join("/")}`
        : null,
    });
  } catch (error) {
    console.error("[qa] failed to load latest QA summary", error);
    sendError(res, 500, "Unable to load QA summary", {
      message: error.message,
    });
  }
});
...
app.post("/api/imports/templates", csrfProtection, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      error.message === "Template payload must be an object"
    ) {
      sendError(res, 422, error.message);
    } else {
      sendError(res, 500, "Unable to persist template", {
        message: error.message,
      });
    }
  }
...
app.put("/api/imports/templates/:id", csrfProtection, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      sendError(res, 500, "Unable to update template", {
        message: error.message,
      });
    }
  }
...
app.delete("/api/imports/templates/:id", csrfProtection, async (req, res) => {
  ...
  try {
    ...
    sendError(res, 500, "Unable to delete template", {
      message: error.message,
    });
  }
});
...
app.post(
  "/api/imports/templates/:id/contracts",
  csrfProtection,
  async (req, res) => {
    const { id } = req.params;
    const formatRaw =
      typeof req.body?.format === "string"
        ? req.body.format.toLowerCase().trim()
        : "yaml";
    ...
    try {
      ...
      const { profile, outputPath, filename } = buildContractOutput({
        template,
        ...
        if (payload.type === "finished") {
          (async () => {
            try {
              ...
              console.error(
                "[contracts] failed to finalise contract job",
                error,
              );
            } finally {
              unsubscribe();
      ...
      sendError(res, 500, "Unable to publish contract", {
        message: error.message,
      });
    }
  },
...
app.post("/api/commands/run", csrfProtection, (req, res) => {
  ...
  const cwd =
    typeof payload.cwd === "string" && payload.cwd.trim().length > 0
      ...
      : undefined;
  const env =
    payload.env && typeof payload.env === "object" ? payload.env : undefined;
  const metadata =
...
app.post("/api/import", csrfProtection, async (req, res) => {
  ...
  if (fileLimitExceeded) {
    ...
    return sendError(
      res,
      413,
      "One or more files exceed the allowed size limit",
      { maxBytes: MAX_IMPORT_FILE_SIZE },
    );
  }
  ...
  if (streamError) {
    ...
    await rm(tempDir, { recursive: true, force: true });
    return sendError(res, 400, "Failed to receive files", {
      message: streamError.message,
    });
  }
  ...
  if (attachProfile) {
    const rawPayload =
      typeof fields.profilePayload === "string"
        ? fields.profilePayload.trim()
        : "";
    if (!rawPayload) {
      await rm(tempDir, { recursive: true, force: true });
      return sendError(
        res,
        400,
        "profilePayload is required when attachProfile is true",
      );
    }
    try {
      ...
      await rm(tempDir, { recursive: true, force: true });
      return sendError(res, 400, "profilePayload must be valid JSON", {
        message: error.message,
      });
    }
  }

  const profileSourceName =
    typeof fields.profileSourceName === "string" &&
    fields.profileSourceName.trim().length > 0
      ? fields.profileSourceName.trim()
      : undefined;

  const profile =
    typeof fields.profile === "string" && fields.profile.trim().length > 0
      ? fields.profile.trim()
      : "generic";
  const notes =
    typeof fields.notes === "string" && fields.notes.trim().length > 0
      ? fields.notes.trim()
      : undefined;
  ...
  try {
      ...
      if (payload.type === "finished") {
        (async () => {
          try {
            ...
            const status =
              typeof payload?.status === "string" ? payload.status : job.status;
    ...
    sendError(res, 500, "Failed to start import job", {
      message: error.message,
    });
  }
});
...
app.get("/api/runs/recent", async (req, res) => {
  ...
  try {
    ...
    const prefectRuns = Array.isArray(prefectRunsRaw)
      ? prefectRunsRaw.map(mapPrefectRunToPipeline)
      : [];
    ...
    sendError(res, 500, "Failed to load recent runs", {
      message: error.message,
    });
  }
});
...
app.get("/api/activity", async (req, res) => {
  try {
    ...
    sendError(res, 500, "Failed to read activity log", {
      message: error.message,
    });
  }
});
...
app.get("/api/activity/events", async (req, res) => {
  ...
  try {
    ...
    res.write(
      `data: ${JSON.stringify({ message: "Failed to load activity history" })}\n\n`,
    );
  }
...
app.post("/api/hil/approvals", csrfProtection, async (req, res) => {
  ...
  if (!status || !hilStatusSet.has(status)) {
    return sendError(
      res,
      422,
      "status must be one of waiting, approved, rejected",
    );
  }
  if (!operator) {
  ...
  try {
    ...
    const approval = createHilApproval({
      runId,
      status,
      operator,
      comment,
    ...
    const updatedAudit = [auditEntry, ...auditLog].slice(
      0,
      Number.parseInt(process.env.HOTPASS_HIL_AUDIT_LIMIT ?? "500", 10),
    );
    ...
    sendError(res, 500, "Failed to update approval", {
      message: error.message,
    });
  }
});
...
app.get("/api/jobs/:id/artifacts", async (req, res) => {
  ...
  try {
    ...
    console.error("[import] failed to enumerate artifacts", error);
    sendError(res, 500, "Failed to enumerate artifacts", {
      message: error.message,
    });
  }
});
...
app.get("/api/jobs/:id/artifacts/profile", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${attachment.name || path.basename(resolvedPath)}"`,
    );
    ...
    sendError(res, 500, "Failed to stream profile artifact", {
      message: error.message,
    });
  }
});
...
app.get("/api/jobs/:id/artifacts/contract", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Type",
      attachment.format === "json" ? "application/json" : "application/x-yaml",
    );
    res.setHeader("Content-Length", stats.size);
    res.setHeader(
      "Content-Disposition",
    ...
    sendError(res, 500, "Failed to stream contract artifact", {
      message: error.message,
    });
  }
});
...
app.get("/api/jobs/:id/artifacts/refined", async (req, res) => {
  ...
  try {
    ...
    sendError(res, 500, "Failed to stream artifact", {
      message: error.message,
    });
  }
});
...
app.get("/health", (_req, res) => {
  ...
  res.json({
    ...
    timestamps: {
      serverStartedAt:
        jobs.length > 0 ? (jobs[jobs.length - 1]?.createdAt ?? null) : null,
