...
| -------------------------------------------------------------------- | ---------- | ---------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Confirm Phase 5 T5.5 completion against programme expectations       |            | Programme        | Complete           | Sign-off recorded after ARC rehearsal artefacts were archived under `dist/staging/arc/20251113T160000Z/` and the roadmap Phase 5 entry updated.                                                             |
| Execute full E2E runs with canonical configuration toggles           |            | QA               | Complete           | Local verification recorded under `dist/logs/e2e-20251107T105820Z/` (overview → refine → enrich → plan research → qa all) using the generic profile + sample `data/e2e` workbook.                           |
...
| Benchmark `HotpassConfig.merge` on large payloads                    |            | Engineering      | Complete           | `scripts/benchmarks/hotpass_config_merge.py` generates `dist/benchmarks/hotpass_config_merge.json` (baseline run: mean merge time ≈1.03 ms with 30 agents).                                                                                    |
| Extend orchestrate/resolve CLI coverage for advanced profiles        |            | Engineering & QA | Complete           | `tests/cli/test_resolve_profile.py`, `tests/cli/test_research_plan.py`, and the new `tests/cli/test_orchestrate.py` exercise advanced Splink/Label Studio flows plus orchestrator feature toggles.                                              |
