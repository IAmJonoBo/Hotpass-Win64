...
def summarise_pnpm_audit(payload: Any) -> dict[str, Any]:
    summary: dict[str, Any] = {
        "total": 0,
        "by_severity": {key: 0 for key in SEVERITY_ORDER},
    }
    ...
    summary["total"] = (
        ...
        else int(vulnerabilities.get("total", 0) or 0)
    )
...
def render_summary(pip_summary: dict[str, Any], pnpm_summary: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("### Web UI (pnpm audit)")
    if pnpm_summary["total"] == 0:
        lines.append("- No known vulnerabilities detected.")
    else:
        lines.append(f"- {pnpm_summary['total']} vulnerabilities detected")
        lines.extend(
            f"  - {severity.title()}: {pnpm_summary['by_severity'][severity]}"
            for severity in SEVERITY_ORDER
            if pnpm_summary["by_severity"][severity] > 0
        )
    lines.append("")
    lines.append(
        "Raw reports are published alongside this release for downstream triage."
    )
    return "\n".join(lines)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render release security summary text."
    )
    parser.add_argument(
        "--pip-audit", type=Path, required=True, help="Path to pip-audit JSON output"
    )
    parser.add_argument(
        "--pnpm-audit", type=Path, required=True, help="Path to pnpm audit JSON output"
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> None:
    ...
    pnpm_payload = load_json(args.pnpm_audit)
    summary = render_summary(
        summarise_pip_audit(pip_payload), summarise_pnpm_audit(pnpm_payload)
    )
